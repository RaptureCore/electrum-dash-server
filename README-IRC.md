IRC is used by Electrum Rapture server to find 'peers' - other Electrum Rapture servers. The
current list can be seen by running:

  electrum-rapture-server peers

The following config file options are used by the IRC part of Electrum Rapture server:
...
the Electrum Rapture server will run in private mode (default). In private
mode, `electrum-rapture-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum Rapture
...
your Electrum Rapture server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-rapture .
