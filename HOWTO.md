How to run your own Electrum Rapture server
========================================
...
Electrum Rapture server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/rapturepay/electrum-rapture-server/blob/master/HOWTO.md
...
document, we assume that user is called 'rapture'). We also assume the
rapture user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run raptured and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit raptured 
...
only leaving spare cycles for raptured. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running raptured and Electrum Rapture server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `raptured` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser rapture --disabled-password
    $ sudo apt-get install git
    $ sudo su - rapture
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/rapture/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download raptured

Recommend downloading latest version directly from GitHub

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - rapture
    $ cd ~/src && wget https://github.com/RaptureCore/Rapture/releases/download/v1.1.2.2/rapturecore-1.1.2-linux64.tar.gz
    $ tar xfz rapturecore-1.1.2-linux64.tar.gz
    $ cd rapturecore-1.1.2/bin
    $ cp -a raptured rapture-cli rapture-tx ~/bin

### Step 3. Configure and start raptured

In order to allow Electrum Rapture to "talk" to `raptured`, we need to set up an RPC
username and password for `raptured`. We will then start `raptured` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.rapturecore
    $ $EDITOR ~/.rapturecore/rapture.conf

Write this in `rapture.conf`:
...
If you have an existing installation of raptured and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ raptured -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `raptured`:

    $ raptured

Allow some time to pass for `raptured` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ rapture-cli getblockchaininfo

Before starting the Electrum Rapture server your raptured should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start raptured at boot
time, running as the 'rapture' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Rapture server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/RaptureCore/electrum-rapture-server.git
    $ cd electrum-rapture-server
...
### Optional Step 5: Install Electrum Rapture dependencies manually

Electrum Rapture server depends on various standard Python libraries and leveldb. These will usually be
...
package manager if you don't want to use the install routine.
    ...
    $ sudo easy_install jsonrpclib irc plyvel coinhash
...
Electrum Rapture server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Rapture full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Rapture server config.
...
### Step 9. Configure Electrum Rapture server

Electrum Rapture reads a config file (/etc/electrum-rapture.conf) when starting up. This
file includes the database setup, raptured RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-rapture.conf
...
Electrum Rapture server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-rapture-server.
If you're using the user `rapture` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "rapture hard nofile 65536" >> /etc/security/limits.conf
     echo "rapture soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Rapture server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u rapture -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close raptured for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting raptured (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Rapture server

The magic moment has come: you can now start your Electrum Rapture server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Rapture server
...
If you want to stop Electrum Rapture server, use the 'stop' command:
...
### Step 12. Test the Electrum Rapture server
...
current server and connect to your new Electrum Rapture server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
raptures to confirm that everything is working properly.
...
If you're operating a public Electrum Rapture server please subscribe
